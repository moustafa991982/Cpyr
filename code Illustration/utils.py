...
def create_dir(tag):
    ...
    if not os.path.exists(newpath):
        os.makedirs(newpath)
        print('model is stored in ',newpath)
    
def save_all(path,**kwargs):
    ...
    model.save(path+model.__name__())
    opt.save(path+opt.__name__())
    if learn.sched !=None:
        learn.sched.save(path+learn.sched.__name__())
    

def load_all(path,mother_path='Models/',**kwargs):
    '''Load all model details in place(model,optimizer,scheduler)'''
    model = kwargs.get('model', '')
    opt = kwargs.get('opt', '')
    learn = kwargs.get('learn','')
    
    path = mother_path + path
