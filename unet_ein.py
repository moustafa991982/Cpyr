...
class Unet_sigmoid(BaseModel):
    def __init__(self,att = False,name = 'U-net_sigmoid',pre=True):
        super().__init__(name)
        self.loss = []
        self.one = []
        ##Load pre-trained
...
class unet_ein(BaseModel):
    
    def __init__(self,bs,seq_len,d_model = 512,pre=True,n_layers=2,out_ch=512,dropout = 0.1):
        super().__init__()
        self.unet = Unet_sigmoid(pre=pre)
        self.ein = e.Ein(bs,seq_len,d_word_vec=d_model , d_model=d_model, d_inner=d_model*4,n_layers=n_layers,
    ...
    def forward(self,x):
        ...
        trans = u[-1].view(bs,ts,u[-1].size(1),u[-1].size(2),u[-1].size(3))
        trans_op = self.ein(trans)
        
        img = u.copy()
        img[-1] = trans_op.view(bs*ts,img[-1].size(1),img[-1].size(2),img[-1].size(3))
        img = self.unet.decode(*img)
        return trans,trans_op,img#,self.unet.unet.named_children
