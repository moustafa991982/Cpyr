...
ISO 21448:2022 requires evidence for three activities: (1) triggering-condition identification, (2) known-unsafe scenario mitigation, (3) residual Zone 2 reduction. MISV (Multiple independent source of verification) contributes to all three:
To make it simple: The entire engineering goal of SOTIF is to shrink Zones 2 and 3 — convert unknowns into knowns (Zone 3 → Zone 2), and convert knowns-unsafe into knowns-safe through mitigation (Zone 2 → Zone 1). 
...
2- Traffic Vision watches the camera — the external visual scene. It flags anomalies as high-MSE video windows (your predictive autoencoder, where a bad future-frame prediction means something unexpected is happening).
